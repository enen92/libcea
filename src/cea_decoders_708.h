...
#ifndef _CEA_DECODERS_708_H
#define _CEA_DECODERS_708_H

#include "cea_common_constants.h"
#include "cea_common_structs.h"

#define CEA_DTVCC_MAX_PACKET_LENGTH 128 // According to EIA-708B, part 5
#define CEA_DTVCC_MAX_SERVICES 63

#define CEA_DTVCC_MAX_ROWS 15
/**
 ...
 */
#define CEA_DTVCC_MAX_COLUMNS (32 * 2)

#define CEA_DTVCC_SCREENGRID_ROWS 75
#define CEA_DTVCC_SCREENGRID_COLUMNS 210

#define CEA_DTVCC_MAX_WINDOWS 8
#define NOT_OVERLAPPED 0
#define OVERLAPPING_WITH_HIGH_PRIORITY 0
#define OVERLAPPED_BY_HIGH_PRIORITY 1

#define CEA_DTVCC_FILENAME_TEMPLATE ".p%u.svc%02u"

#define CEA_DTVCC_NO_LAST_SEQUENCE -1
...
#define CEA_DTVCC_SYM_SET(x, c) \
	{                       \
		x.init = 1;     \
		x.sym = c;      \
	}
#define CEA_DTVCC_SYM_SET_16(x, c1, c2) \
	{                               \
		x.init = 1;             \
		x.sym = (c1 << 8) | c2; \
	}
#define CEA_DTVCC_SYM(x) ((unsigned char)(x.sym))
#define CEA_DTVCC_SYM_IS_EMPTY(x) (x.init == 0)
#define CEA_DTVCC_SYM_IS_SET(x) (x.init == 1)
...
typedef struct dtvcc_window
{
	...
	int pen_row;
	int pen_column;
	dtvcc_symbol *rows[CEA_DTVCC_MAX_ROWS];
	dtvcc_pen_color pen_colors[CEA_DTVCC_MAX_ROWS][CEA_DTVCC_SCREENGRID_COLUMNS];
	dtvcc_pen_attribs pen_attribs[CEA_DTVCC_MAX_ROWS][CEA_DTVCC_SCREENGRID_COLUMNS];
...
{
	dtvcc_symbol chars[CEA_DTVCC_SCREENGRID_ROWS][CEA_DTVCC_SCREENGRID_COLUMNS];
	dtvcc_pen_color pen_colors[CEA_DTVCC_SCREENGRID_ROWS][CEA_DTVCC_SCREENGRID_COLUMNS];
	dtvcc_pen_attribs pen_attribs[CEA_DTVCC_SCREENGRID_ROWS][CEA_DTVCC_SCREENGRID_COLUMNS];
	int64_t time_ms_show;
	int64_t time_ms_hide;
 ...
 */
typedef struct cea_decoder_dtvcc_report
{
	int reset_count;
	unsigned services[CEA_DTVCC_MAX_SERVICES];
} cea_decoder_dtvcc_report;

typedef struct dtvcc_service_decoder
{
	dtvcc_window windows[CEA_DTVCC_MAX_WINDOWS];
	int current_window;
	dtvcc_tv_screen *tv;
	int cc_count;
} dtvcc_service_decoder;

typedef struct cea_decoder_dtvcc_settings
{
	int enabled;
...
typedef struct dtvcc_ctx
{
	int is_active;
	int active_services_count;
	int services_active[CEA_DTVCC_MAX_SERVICES]; // 0 - inactive, 1 - active
	int report_enabled;

	cea_decoder_dtvcc_report *report;

	dtvcc_service_decoder decoders[CEA_DTVCC_MAX_SERVICES];

	unsigned char current_packet[CEA_DTVCC_MAX_PACKET_LENGTH];
